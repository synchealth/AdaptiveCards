...
| **selectAction** | `any` |  No | An Action that will be invoked when the `Image` is tapped or selected. `Action.ShowCard` is not supported. | 1.1 |
| **size** | `any` |  No | Controls the approximate size of the image. The physical dimensions will vary per host. Specify `"auto"` for true image dimension, or `"stretch"` to force it to fill the container. | 1.0 |
| **style** | `any` |  No | Controls how this `Image` is displayed. | 1.0 |
